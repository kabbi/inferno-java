...
{#>>
	...
	# the stream. Next skip the bytes then return the "actual" bytes skipped.
        ...
        if (this.fd.fd == nil)
                jni->ThrowException("java.lang.NullPointerException", "null FileDescriptor");
        cur := sys->seek(this.fd.fd, big 0, sys->SEEKRELA);
	end := sys->seek(this.fd.fd, p0, sys->SEEKRELA);
        return big (end - cur);
}#<<
...
{#>>
        if (this.fd.fd == nil)
                jni->ThrowException("java.lang.NullPointerException", "null FileDescriptor");
        cur := sys->seek(this.fd.fd, big 0, Sys->SEEKRELA);
        end := sys->seek(this.fd.fd, big 0, Sys->SEEKEND);
        sys->seek(this.fd.fd, cur, Sys->SEEKSTART);

        return int (end - cur);
}#<<

close0_V( this : ref FileInputStream_obj)
{#>>
	if (this.fd != nil)
		this.fd.fd = nil;
}#<<
